...
statsRouter.get("/:campaignID/email/comprehensive", async (req: Request, res: Response) => {
    const { campaignID } = req.params;
    ...
    try {
        ...
        emailCampaigns.forEach((emailCampaign) => {
            if(emailCampaign.targeted==0) return res.json({
                message : "No emails have been sent"
            })
            const emaildata = {
                title : emailCampaign.title,
                opened : emailCampaign.opened,
                targeted : emailCampaign.targeted,
                perc_success : emailCampaign.opened/emailCampaign.targeted
            }
